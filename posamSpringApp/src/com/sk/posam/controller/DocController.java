...
public class DocController {
	...
	@PostMapping("/scan")
	public String scan(Model model, String inputGroupSelect02, String name, String password) throws IOException {
		...
		if(scanner.OpenDevice()){
			...
			//	zobrazenie odtlačku na vedlajšom panely, defaultne alebo ked je SHOWING
			...
			//registration
				...
				if(name.length()==0 || password.length()==0) {
					...
					try {
						...
						byte[] array = ((DataBufferByte) image1.getRaster().getDataBuffer()).getData();
						System.arraycopy(data, 0, array, 0, array.length);
						File outputfile = new File("/Users/user/Desktop/PoSam/posamSpring/posamSpringApp/fingerprints/" + password + ".png");
						ImageIO.write(image1, "png", outputfile);
					}catch (IOException e){
						System.out.println(e);
					}

					// usersStorageService.saveUser(name, username, data, image);
					s = "Registrácia užívateľa a pridanie odtlačku prebehlo úspešne !";
				}
				model.addAttribute("vysledok",s);
			}
			else if (inputGroupSelect02.equals("login")){

			}
			else if(inputGroupSelect02.equals("identification")) {
				FingerprintTemplate probe = new FingerprintTemplate(
						new FingerprintImage(
								Files.readAllBytes(Paths.get("/Users/user/Desktop/PoSam/posamSpring/posamSpringApp/fingerprints/kok.png")),
								new FingerprintImageOptions().dpi(500)));

				FingerprintTemplate candidate = new FingerprintTemplate(
						new FingerprintImage(
								Files.readAllBytes(Paths.get("/Users/user/Desktop/PoSam/posamSpring/posamSpringApp/fingerprints/lof.png")),
								new FingerprintImageOptions().dpi(500)));
